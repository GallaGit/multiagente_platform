# O01 — Sistematización de la captación de propietarios

**Categoría:** Captación  
**Clasificación:** Respaldada  
**Nivel de evidencia:** Medio  
**Nivel de confianza:** Medio

## 1. Descripción

Ordenar prospección, seguimiento y evidencia de valor en un contexto de oferta escasa, sin asumir que la tecnología crea stock. Aparece en **P01, P02** y deriva de **D01, D04**. Delimita una oportunidad, no un producto.

## 2. Problemas relacionados

- **D01, D04.** [Índice del Módulo 4](../../Modulo-04-Dolor-del-Cliente/README.md#índice-de-problemas).
- Frecuencia, impacto y urgencia son cualitativos; no tasas nacionales.

## 3. Procesos afectados

- **P01, P02.** [Taxonomía](../Metodologia.md#taxonomía-de-trazabilidad).
- [Funcionamiento](../../analisis_del_mercado/02-funcionamiento.md) · [Flujo lead–postventa](../../ecosistema_tecnologico/01_arquitectura_y_flujos/flujo-lead-postventa.md).

## 4. Herramientas implicadas

- **H01, H05, H06, H07.** [Catálogo normalizado](../Metodologia.md#taxonomía-de-trazabilidad).

## 5. Competencia existente

- **Competidores:** C01, C02.
- **Cobertura y límites:** Inmovilla y Witei cubren CRM; idealista/tools vincula captación al portal. No resuelven escasez estructural, saturación del propietario ni demuestran conversión por canal.
- **Sin resolver:** cobertura extremo a extremo, adopción efectiva y resultado por segmento.
- [Módulo 5](../../Modulo-05-Competencia/README.md) · [Comparativa M3](../../ecosistema_tecnologico/07_analisis_transversal/comparativa-proveedores.md).

## 6. Personas beneficiadas

Gerencia, captadores, marketing y propietarios.

## 7. Beneficio esperado

Menos seguimientos omitidos y tiempo de prospección; mayor continuidad y mejor experiencia. No se asigna ROI sin medición de campo.

## 8. Complejidad estimada

**Media.** Usa capacidades maduras, pero exige consolidar canales y disciplina comercial.

## 9. Nivel de innovación

**Incremental.** Amplía o hace más fiable un proceso ya atendido por herramientas existentes; no cambia la lógica de intermediación.

## 10. Evidencia y validación

- **Evidencias:** D01, D04; P01, P02; H01, H05, H06, H07; C01, C02; [vacíos tecnológicos](../../ecosistema_tecnologico/07_analisis_transversal/vacios-tecnologicos.md).
- **Calidad:** fichas trianguladas del M4 y documentación del M3; detalle competitivo del M5 incompleto.
- **Limitaciones:** Faltan coste por captación, intentos por encargo y conversión española por canal.
- **Pendiente:** Medir coste y conversión por canal; entrevistar agencias y propietarios.

## 11. Riesgos

| Riesgo | Evaluación |
|---|---|
| Tecnológico | Media: integración, datos y terceros según alcance. |
| Comercial | Medio: disposición a pagar no demostrada. |
| Regulatorio | Medio; alto si trata identidad, contratos, pagos o datos sensibles. |
| Adopción | Medio-alto: depende de registro consistente y hábitos. |
| Competitivo | Medio-alto: proveedores existentes tienen distribución y datos. |

## 12. Criterio de clasificación

**Respaldada.** Problema, proceso y vacío funcional triangulados; demanda y economía siguen pendientes.

---

[Índice](../README.md) · [Cobertura](../Analisis/Mapa-de-Cobertura.md) · [Riesgos](../Analisis/Riesgos.md)
